# pso_singlefigure_alliters_arrows_with_panel_and_wcurve.py
import numpy as np
...
# -------------------- Logging helper (optional console log) --------------------
...
# -------------------- PSO with full histories for animation --------------------
def pso_with_history(Positions, lowerbound, upperbound, iters=100, seed=7, log_first_n=3):
    """
    4 particles, 2D PSO.
    Returns:
        P_hist        : list of (4x2) positions at frames 0..iters
        V_hist        : list of (4x2) velocities at frames 0..iters
        pbestX_hist   : list of (4x2) personal-best positions at frames 0..iters
        pbestF_hist   : list of (4,)  personal-best fitnesses at frames 0..iters
        g_hist        : list of (2,)  global-best positions at frames 0..iters
        gbestF_hist   : list of ()    global-best fitness at frames 0..iters
        w_hist        : list of inertia values aligned with frames (length = iters+1)
    """
    rng = np.random.default_rng(seed)
    n, dim = Positions.shape
    assert n == 4 and dim == 2, "This demo assumes exactly 4 particles in 2D."
    ...
    # Histories (frame 0 = initial state)
    P_hist      = [P.copy()]
    V_hist      = [V.copy()]
    ...
    g_hist      = [gbest_x.copy()]
    gbestF_hist = [gbest_f]
    w_hist      = [w_max]  # define w at frame 0 as w_max

    # Iterate
    for t in range(1, iters + 1):
        # inertia schedule: w(t) = 0.9 - t*((0.9-0.4)/iters)
        w = w_max - t * ((w_max - w_min) / iters)

        # Pre-generate r1, r2 for reproducibility/logging
...
if __name__ == "__main__":
    ...
    # Four (x,y) pairs for initialization
    FOUR_POS = np.array([
        [-2.2,  2.5],
        [ 1.6, -1.8],
        [-0.7,  0.4],
    ...
    # -------- Figure layout: [3D | 2D | w-curve] on top, status panel bottom --------
    ...
    # Status panel (bottom; text-only axis)
    ...
    # Arrow holders (module-level lists; no nonlocal/global rebinding)
    arrows3d = []
    arrows2d = []
    ...
    # Build status string for a given frame
    def build_status_string(frame: int) -> str:
        # frame ∈ [0, ITERATIONS]; frame 0 is the initial state
        pop    = P_hist[frame]
        vel    = V_hist[frame]
        ...
        if frame == ITERATIONS:
            ...
            lines.append(f"Optimal (found): gbest=({gb[0]:+7.6f},{gb[1]:+7.6f})   f={gbF:0.8f}")
    ...
    # Update for frame t = 0..ITERATIONS
    def update_anim(frame):
        ...
        # Update status panel and w-marker
    ...
    # Save GIF
    writer = PillowWriter(fps=12, metadata={"artist": "PSO demo"})
    def _progress(i, n):
        if i % max(1, n // 10) == 0 or i == n-1:
            print(f"Saving GIF: frame {i+1}/{n}")
    anim.save("pso_all_iterations_with_panel.gif", writer=writer, dpi=120, progress_callback=_progress)
